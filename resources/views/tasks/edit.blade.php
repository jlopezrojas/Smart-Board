...
@section('content')
    ...
    <div class="row">
        <form action="{{ route('tasks.update', $task->id) }}" method="POST" class="col s12">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="input-field col s12">
                    <input id="nombre" type="text" name="nombre" class="validate" value="{{ $task->nombre }}">
                    <label for="nombre">Nombre</label>
                </div>
            </div>
            <div class="row">
                <div class="input-field col s12">
                    <textarea id="descripcion" class="materialize-textarea" name="descripcion">{{ $task->descripcion }}</textarea>
                    <label for="descripcion">Descripción</label>
                </div>
            </div>
            <div class="row">
                <div class="input-field col s12 m6">
                    <input id="asignatario" type="text" name="asignatario" class="validate" value="{{ $task->asignatario }}">
                    <label for="asignatario">Asignatario</label>
                </div>
                <div class="input-field col s12 m6">
                    <input id="costo" type="number" name="costo" class="validate" value="{{ $task->costo }}">
                    <label for="costo">Costo</label>
                </div>
            </div>
            <div class="row">
                <div class="input-field col s12 m6">
                    <input id="fecha_inicio" type="date" name="fecha_inicio" class="datepicker" value="{{ $task->fecha_inicio }}">
                    <label for="fecha_inicio">Fecha de Inicio</label>
                </div>
                <div class="input-field col s12 m6">
                    <input id="fecha_termino" type="date" name="fecha_termino" class="datepicker" value="{{ $task->fecha_termino }}">
